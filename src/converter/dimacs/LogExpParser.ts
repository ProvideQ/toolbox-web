...
export class LogExpParser {
    ...
    private getOperatorPositions(tokens: LeveledToken[], operator: string, level: number): number[] {
    ...
    private processOR(tokens: LeveledToken[]): LeveledToken[] {
        ...
        for (let pos of operatorPositions) {
            ...
            let processedBefore = before[0].level != level ? this.processOR(before) : this.processAND(before);
            returnTokens.push(...processedBefore);
        }
        if (operatorPositions.length > 1) this.output += `)`;
